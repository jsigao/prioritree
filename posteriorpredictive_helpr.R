  ...
  for (i in 1:nsim) {
    ...
    for (j in 1:nrow(tree$edge)) {
      if (tree$edge.length[j] == 0) {
        ...
        names(map)[1] <- node.states[which(tree$edge[, 
                                                     1] == tree$edge[j, 2]), 1] <- node.states[j, 
                                                                                               2] <- node.states[j, 1]
      }
      else {
        time = 0
        ...
        while (time < tree$edge.length[j]) {
          ...
          if (Q[state, state] == 0) {
            ...
            dt[2] <- dt[1] + rexp(n = 1, rate = -Q[state, 
                                                   state])
          }
          
          if (dt[2] < tree$edge.length[j]) 
            new.state <- rstate(Q[, state][-match(state, 
                                                  rownames(Q))]/sum(Q[, state][-match(state, 
                                                                                      rownames(Q))]))
        ...
        node.states[which(tree$edge[, 1] == tree$edge[j, 
                                                      2]), 1] <- node.states[j, 2] <- names(map)[length(map)]
      }
      mtree$maps[[j]] <- map
    }
    mtree$node.states <- node.states
    tip.states <- node.states[tree$edge[, 2] <= length(tree$tip), 
                              2]
    tip.states <- tip.states[order(tree$edge[tree$edge[, 
                                                       2] <= length(tree$tip), 2])]
    names(tip.states) <- tree$tip.label
    mtree$states <- tip.states
    allstates <- vector()
    for (j in 1:nrow(mtree$edge)) allstates <- c(allstates, 
                                                 names(mtree$maps[[j]]))
    allstates <- unique(allstates)
    mtree$mapped.edge <- matrix(data = 0, length(mtree$edge.length), 
                                length(allstates), dimnames = list(apply(mtree$edge, 
                                                                         1, function(x) paste(x, collapse = ",")), state = allstates))
    for (j in 1:length(mtree$maps)) for (k in 1:length(mtree$maps[[j]])) mtree$mapped.edge[j, 
                                                                                           names(mtree$maps[[j]])[k]] <- mtree$mapped.edge[j, 
  ...
  while ((simsamples_num == 0) || (simsamples_id <= simsamples_num)) {
    ...
    if (length(log_line) == 0) {
      ...
      if (length(logline_values) == length(col_names)) {
        ...
        # todo: providing a toggle to choose to use phytools:::statdist to compute stationary freq and then using it to rescale correctly
        # however, I'm not sure if the "correct rescaling" is correct to do here, as this de-rescaling step needs to be exactly reverse to
        # how rescaling was performed
